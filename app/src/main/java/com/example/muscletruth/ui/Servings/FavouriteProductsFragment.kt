...
class FavouriteProductsFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        productsList = view.findViewById<RecyclerView>(R.id.fgt_favourite_products_products_rv)
        setupList()
        loadData()
        ...
        val addButton = view.findViewById<Button>(R.id.fgt_favourite_products_btn_add)
        addButton.setOnClickListener{
            val intent = Intent(requireContext(), AddProductActivity::class.java)
            addProductLauncher.launch(intent)
        }

        val backButton = view.findViewById<Button>(R.id.fgt_favourite_products_btn_back)
        ...
        lifecycleScope.launch {
            try{
                products = withContext(Dispatchers.IO){
                    val favouriteProducts = ProductRepository.getFavouriteProducts()
                    Log.d("APP_DEBUG!", "FAVOURITE2 $favouriteProducts")
                    favouriteProducts.map{it -> ProductRepository.getProduct(it.productServerID, it.productLocalID)!!}.toMutableList()
                }
                Log.d("APP_DEBUG!", "FAVOURITE $products")
    ...
    private fun setupList() {
        ...
        }, requireContext(), lifecycleScope, {loadData()})
